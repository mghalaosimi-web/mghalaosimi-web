...
graph TD
    ...
    Gateway --> Auth[Identity & Access Management]
    Gateway --> Core[Core ERP Services]
    Gateway --> OCR[OCR Parsing Engine]
    Gateway --> AI[AI Predictive Layer]
    Core --> Cache[(Redis Cache)]
    Core --> DB[(Distributed PostgreSQL)]
    OCR --> MQ[Kafka Message Queue]
    AI --> MQ
    MQ --> Async[Async Job Processors]
```
